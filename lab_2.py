...
tagWordList = getTaggedWordsFromFile(taggedBrown)
...
wordList = getTaggedWordsFromFile(taggedBrown)
nOWords = len(wordList)
ubtGrams = countNgramsPWW(wordList, 0)
...
for tri in tqdm(ubtGrams[2]):
	...
	pYGivenX = float(ubtGrams[1][(x, y)]) / ubtGrams[0][x]
	pZGivenXY = float(ubtGrams[2][(x, y, z)]) / ubtGrams[1][(x, y)]
	HPww += - (pX * pYGivenX * pZGivenXY * log(pZGivenXY, 2))
print len(ubtGrams[2]), 'trigrams.'
print 'Entropy of trigrams full corpus PWW: ', HPww
print 'Perplexity trigrams full corpus PWW: ', 2 ** HPww


ubtGrams = countNgramsPPW(wordList, 0)
HPww = 0
for tri in tqdm(ubtGrams[2]):
	x = tri[0]
	y = tri[1]
	...
	HPww += - (pX * pYGivenX * pZGivenXY * log(pZGivenXY, 2))
print len(ubtGrams[2]), 'trigrams.'
print 'Entropy of trigrams full corpus PPW: ', HPww
print 'Perplexity trigrams full corpus PPW: ', 2 ** HPww
